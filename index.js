  ...
  if (list.dataset.mode ==='modeCard'){
    ...
    <div class="card m-2 text-center" style="width: 18rem"  data-id='${element.id}'>
      ...
      <div class="card-body" data-id='${element.id}'>
        ...
        <i class="bi ${heart}" data-id='${element.id}'></i>
      </div>
    </div>
    `;
      heart = 'bi-heart'
  ...
  if (page >= 5) {
    ...
    <li class="page-item" data-name='${dataName}'>
      <a class="page-link" href="#" aria-label="Previous" id='${previousPage}' data-name='${dataName}'>
        <span aria-hidden="true" id='${previousPage}'  data-name='${dataName}'>&laquo;</span>
      </a>
    </li>
    ...
    <li class="page-item" data-name='${dataName}'>
      <a class="page-link" href="#" aria-label="Next" id='${nextPage}' data-name='${dataName}'>
        <span aria-hidden="true" id='${nextPage}'  data-name='${dataName}'>&raquo;</span>
      </a>
    </li>
  `
  } else {
...
list.addEventListener("click", function(event) {
  let favoriteData = JSON.parse(localStorage.getItem('favoriteData')) || []
  if (event.target.matches('.bi-heart')){
    event.target.classList.remove('bi-heart')
    event.target.classList.add('bi-heart-fill')
    let data = userList.find(function(item){
    ...
    let index = favoriteData.findIndex(function (item) {
      return item.id === Number(event.target.dataset.id)
    })
